# Unsafe .NET 3.5 实现 / Unsafe .NET 3.5 Implementation

## 概述 / Overview

本项目为 .NET Framework 3.5 提供了 `System.Runtime.CompilerServices.Unsafe` 类的完整实现，包含 34 个静态方法，支持高性能的内存操作和指针运算。

This project provides a complete implementation of the `System.Runtime.CompilerServices.Unsafe` class for .NET Framework 3.5, including 34 static methods supporting high-performance memory operations and pointer arithmetic.
...
- 需要 .NET Framework 3.5 或更高版本
- 需要启用 unsafe 代码编译选项
- 某些方法返回 void* 而不是 ref T（.NET 3.5 限制）

Requires .NET Framework 3.5 or higher.
Requires unsafe code compilation option.
Some methods return void* instead of ref T (.NET 3.5 limitation).
